...
class Gdpr_Cookie_Consent_Policy_Data {
	...
	public function gdpr_save_custom_metabox() {
        global $post;
		if ( isset( $_POST['_gdpr_policies_domain'] ) ) {
			update_post_meta( $post->ID, '_gdpr_policies_domain', sanitize_text_field( $_POST['_gdpr_policies_domain'] ) );
		}
		if (isset($_POST['_gdpr_policies_links_editor']))
		{
			$data = wp_kses_post($_POST['_gdpr_policies_links_editor']);
		...
		switch ( $column ) {
			case 'title':
				if ( isset( $post->post_title ) ) {
					echo esc_attr_e( $post->post_title, 'gdpr-cookie-consent' );
				}
				break;
			case 'gdprpurpose':
				if ( isset( $post->post_content ) ) {
					echo esc_attr_e( $post->post_content, 'gdpr-cookie-consent' );
				}
				break;
			case 'gdprlinks':
			...
			case 'gdprdomain':
				...
				if ( isset( $custom['_gdpr_policies_domain'][0] ) ) {
					echo esc_attr_e( $custom['_gdpr_policies_domain'][0], 'gdpr-cookie-consent' );
				}
				break;
	...
	public function gdpr_add_policies_import_button() {
		...
		$import_page      = admin_url( 'edit.php?post_type=' . GDPR_POLICY_DATA_POST_TYPE . '&page=gdpr-policies-import' );
		?>
	...
	public function gdpr_add_policies_export_button() {
        ...
        <script type="text/javascript">
            jQuery(document).ready( function($)
            {
                jQuery("<a  href='<?php echo admin_url( 'admin-post.php?action=gdpr_policies_export.csv' ); ?>' id='export_gdpr_policies' class='add-new-h2'><?php echo $scan_export_menu; ?></a>").insertAfter(".wrap h1");
            });
        </script>
		<?php
    }
	...
	public function gdpr_process_csv_policies() {
		if ( isset( $_POST['_wpnonce-gdpr-policies-import-page'] ) ) {
			...
			if ( ! empty( $_FILES['policies_csv']['tmp_name'] ) ) {
				...
				$filename = $_FILES['policies_csv']['tmp_name'];

				$result = $this->gdpr_import_csv_policies( $filename, array() );
	...
	public function gdpr_import_csv_policies( $filename, $args ) {
		...
		if ( $file_handle ) {
			...
			while ( ( $line = $csv_reader->get_row() ) !== null ) {
				...
				foreach ( $line as $ckey => $column ) {
					...
					$column      = trim( $column );
					if ( in_array( $column_name, $gdprpolicies_fields ) ) {

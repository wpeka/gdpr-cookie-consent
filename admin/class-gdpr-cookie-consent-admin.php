...
class Gdpr_Cookie_Consent_Admin
{
	...
	public function wpl_data_req_process_resolve()
	{
		...
		if (
			isset($_GET['page']) && ($_GET['page'] == 'gdpr-cookie-consent')
			&& isset($_GET['action'])
			&& $_GET['action'] == 'resolve'
			&& isset($_GET['id'])
		) {
	...
	public function gdpr_policy_process_delete()
	{
		...
		if (is_user_logged_in() && current_user_can('manage_options')) {
			...
			if (
				...
				&& wp_verify_nonce($_GET['_wpnonce'], 'gdpr_policy_delete_nonce_' . $_GET['id'])
			) { // Verify nonce using the ID appended to the nonce name
	...
	public function gdpr_cookie_consent_ajax_save_settings()
	{
		...
		if (isset($_POST['gcc_settings_form_nonce'])) {
			...
			if (
				isset($_POST['gcc-ab-testing-enable'])

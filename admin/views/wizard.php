			...
			<div v-show = "show_cookie_as == 'widget' || show_cookie_as == 'popup' || '<?php echo esc_js($template['name']); ?>' !== 'blue_full'" class="gdpr-template-field gdpr-<?php echo esc_attr( $template['name'] ); ?>">
				...
				<div :class=" 'gdpr-right-field template-type-' + show_cookie_as ">
						<div style = "<?php echo esc_attr($styles_attr); ?>" class="cookie_notice_content">
							<span style="display: inline-flex; align-items: center; justify-content: center; position: absolute; top:10px; right: 10px; height: 15px; width: 15px; border-radius: 50%; color: <?php echo esc_attr( $template['accept_button']['background-color'] ); ?>; background-color: transparent;">
							...
							<?php if ( isset( $template['logo']['src'] ) && is_string( $template['logo']['src'] ) && $template['logo']['src'] !== '' ){ ?><img alt="WPCC Logo image" style="<?php echo esc_attr( $logo_style_attr ); ?>" class="gdpr_logo_image" src="<?php echo esc_url( GDPR_COOKIE_CONSENT_PLUGIN_URL . 'includes/templates/logo_images/' . sanitize_file_name( $template['logo']['src'] ) ); ?>">
								<?php }else { ?>
												...
												<div class="gdpr-data-req-email-container">
													...
													<div class="gdpr-data-req-email-subject">
														...
														<c-col class="col-sm-12 gdpr-data-req-subject-input">
															...
															<label for="subject-input" class="screen-reader-text"><?php esc_attr_e( 'Email Subject', 'gdpr-cookie-consent' ); ?></label>

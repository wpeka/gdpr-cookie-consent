...
class Test_Gdpr_Cookie_Consent extends WP_UnitTestCase {
	...
	public function test_gdpr_envelope_settings_tabcontent() {
		...
		self::$gdpr_cookie_consent->gdpr_envelope_settings_tabcontent( $class = 'abc', $target_id = 'abc', $view_file = '', $html = '', $variables, $need_submit_btn = 1, $error_message = '' ); //phpcs:ignore Squiz.PHP.DisallowMultipleAssignments.Found
		$this->assertTrue( true );
		$view_file = plugin_dir_path( GDPR_COOKIE_CONSENT_PLUGIN_FILENAME ) . 'admin/views/admin-display-design.php';
		self::$gdpr_cookie_consent->gdpr_envelope_settings_tabcontent( $class = 'abc', $target_id = 'abc', $view_file = 'admin/views/admin-display-save-button.php', $html = '', $variables, $need_submit_btn = 2, $error_message = '' ); //phpcs:ignore Squiz.PHP.DisallowMultipleAssignments.Found
		$this->assertTrue( true );
	}
